...
const gunzipAsync = promisify.cb_func(zlib.gunzip);
const inflateAsync = promisify.cb_func(zlib.inflate);
...
export class RawAPI extends EventEmitter {
  constructor(opts = {}) {
    ...
    this.raw = {
      ...
      register: {
        ...
        submit(username, email, password, modules) {
          return self.req(
            "POST",
            "/api/register/submit",
            { username, email, password, modules },
          );
        },
      },
      userMessages: {
      ...
      game: {
        ...
        createFlag(
          ...
          return self.req(
            ...
            { name, room, x, y, color, secondaryColor, shard },
          );
        },
        ...
        checkUniqueFlagName(name, shard = DEFAULT_SHARD) {
          return self.req(
            "POST",
            "/api/game/check-unique-flag-name",
            { name, shard },
          );
        },
        ...
        createConstruction(
          ...
          return self.req(
            ...
            { room, x, y, structureType, name, shard },
          );
        },
        ...
        createInvader(
          ...
          return self.req(
            ...
            { room, x, y, size, type, boosted, shard },
          );
        },
         ...
         * @param {string} shard 
         * @returns {Promise<{ok: number, terrain: API.TerrainEncoded}>}
         */
        ...
        market: {
          ...
          myOrders() {
            return self.req("GET", "/api/game/market/my-orders").then(
              self.mapToShard,
            );
          },
      ...
      user: {
        ...
        decorations: {
          ...
          deactivate(decorations) {
            return self.req(
              "POST",
              "/api/user/decorations/deactivate",
              { decorations },
            ); // decorations is a string array of ids
          },
        },
        ...
        rooms(id) {
          return self.req("GET", "/api/user/rooms", { id }).then(
            self.mapToShard,
          );
        },
      ...
      experimental: {
        pvp(interval = 100) {
          return self.req("GET", "/api/experimental/pvp", { interval }).then(
            self.mapToShard,
          );
        },
        nukes() {
          return self.req("GET", "/api/experimental/nukes").then(
            self.mapToShard,
          );
        },
      },
   ...
   * @param {object} opts 
   * @returns {Promise<{ ok: number, token: string }>}
   */
  ...
  async req(method, path, body = {}) {
    ...
    try {
      ...
      if (
        typeof res.data.data === "string" && res.data.data.slice(0, 3) === "gz:"
      ) {
      ...
      if (
        res.status === 429 && !res.headers["x-ratelimit-limit"] &&
